...
export function ShadowTimeControl({ value, onCommit }: { value: Date; onCommit: (minutes: number) => void }) {
  ...
  return <section className="shadow-time" aria-label="出発時刻"><div><span>出発</span><strong>{format(draft)}</strong></div><input type="range" min="540" max="1090" step="10" value={draft} onChange={(event) => setDraft(Number(event.target.value))} onPointerUp={commit} onKeyUp={commit} aria-label="出発時刻を変更" /><div className="time-presets">{presets.map((preset) => <button key={preset.label} onClick={() => { const minutes = preset.minutes(); setDraft(minutes); onCommit(minutes) }}>{preset.label}</button>)}</div><small>スライダーを離すと、影の世界を再計算</small></section>
}
