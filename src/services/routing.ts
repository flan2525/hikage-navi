...
function mapFeature(feature: OrsFeature, index: number): RoutePlan | null { const coordinates = feature.geometry?.coordinates?.map(([lng, lat]) => ({ lng, lat })).filter((p) => Number.isFinite(p.lng) && Number.isFinite(p.lat)) ?? []; if (coordinates.length < 2) return null; const summary = feature.properties?.summary; return { id: `ors-${index}`, kind: index === 0 ? 'shortest' : 'shade', label: index === 0 ? '灼熱ルート' : '影渡り', coordinates, distanceMeters: summary?.distance ?? 0, durationSeconds: summary?.duration ?? 0, source: 'api' } }
...
function sampleRoutes(start: Position, end: Position): RoutePlan[] {
  ...
  return [
    ...
    { id: `sample-shortest-${start.lng}-${end.lng}`, kind: 'shortest', label: '灼熱ルート（実証用サンプル）', source: 'fallback', distanceMeters: directDistance, durationSeconds: directDistance / 1.25, coordinates: [start, middle, end] },
  ]
}
export async function loadRoutes(start: Position = START, end: Position = DESTINATION, signal?: AbortSignal): Promise<{ routes: RoutePlan[]; isFallback: boolean; fallbackReason?: string; requestMilliseconds: number }> { const startedAt = performance.now(); try { return { routes: await fetchWalkingRoutes(start, end, signal), isFallback: false, requestMilliseconds: performance.now() - startedAt } } catch (error) { return { routes: sampleRoutes(start, end), isFallback: true, fallbackReason: error instanceof Error ? error.message : '徒歩経路を取得できませんでした', requestMilliseconds: performance.now() - startedAt } } }
