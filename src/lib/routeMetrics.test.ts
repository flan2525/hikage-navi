...
const result = (statuses: Array<'shaded' | 'sunny' | 'indeterminate'>): ShadeResult => ({ points: statuses.map((status, index) => ({ lng: 132.46, lat: 34.39, status, shaded: status === 'shaded', distanceFromStart: index * 10, arrivalAt: new Date() })), shadedDistanceMeters: 0, totalDistanceMeters: (statuses.length - 1) * 10, evaluatedDistanceMeters: (statuses.length - 1) * 10, outsideCoverageDistanceMeters: 0, shadePercent: 0, sunAltitude: 45, sunBearing: 180, audit: { shadowBearing: 0, routeSampleCount: statuses.length, shadedPointCount: 0, sunnyPointCount: 0, indeterminatePointCount: 0, outsideCoveragePointCount: 0, candidateBuildingCount: 0, shadowPolygonCount: 0, processingMilliseconds: 0 } })
